...
// Helper function to calculate age from a birth date (LocalDate string)
...
/**
 * @typedef {object} PlayerResponseDto
 * @property {number} [id]
 * @property {string} firstName
 * @property {string} lastName
 * @property {string} [avatarUrl]
 * @property {string} birthDate - A string representing LocalDate (e.g., "YYYY-MM-DD")
 ...
 * @param {{ player: PlayerResponseDto }} props
 */
const PlayerHeader = ({ player }) => {
  ...
  // --- Using DTO data and hardcoded placeholders ---
  const playerNumber = player.jerseyNumber;
  const dominantFoot = player.dominantFoot;
  const playerHeight = player.heightCm;
      ...
      <div className="relative px-4 md:px-8 pb-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end gap-4 md:gap-6 -mt-16 md:-mt-24">
          ...
          <div className="flex-shrink-0">
            {/* If avatarUrl exists, use it. Otherwise, use the initial placeholder. */}
            {player.avatarUrl ? (
                <img 
                    src={player.avatarUrl} 
                    alt={displayName} 
                    className="w-32 h-32 md:w-48 md:h-48 rounded-full ring-4 ring-gray-900 shadow-2xl object-cover"
                />
            ) : (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-gray-700">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-white text-base md:text-lg font-semibold mt-1">
              {playerHeight ? `${playerHeight} cm` : "N/A"}/{player.weightKg ? `${player.weightKg} kg` : "N/A"}
            </span>
          </div>
